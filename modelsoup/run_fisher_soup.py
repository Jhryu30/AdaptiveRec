...
def run_fisher_model_soup(model=None, dataset=None, config_file_list=None, config_dict=None, saved=True):
    ...
    config['log_root'] = './log/modelsoup/fisher/'
    ...
    for recipe in MODEL_RECIPE:
        ...
        recipe_fisher_weight = recipe_trainer.compute_fisher_train(eval_data=valid_data, load_best_model=False, show_progress=config['show_progress'])
    ...
    fisher_trainer.save_model_file =os.path.join(config['log_dir'],'fisher_model.pth')
    fisher_trainer._save_checkpoint(epoch=0)
    
    # model fiser finetune
    fisher_trainer.save_model_file =os.path.join(config['log_dir'],'fisher_finetune_model.pth')
    best_valid_score, best_valid_result = fisher_trainer.fit_fisher(
