...
do
    CUDA_VISIBLE_DEVICES=0 python run_seq.py --dataset=$dataset --train_batch_size=$batch_size --model='AdaptiveRec' --contrast=$contrast --sim='cos' --gpu_id=0 \
                                                --lmd=0.1 --lmd_sem=0.1 --tau=1 \
                                                --name='AdaptiveRec'\_$contrast
done
