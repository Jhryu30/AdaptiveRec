...
class SequentialDataLoader(AbstractDataLoader):
    ...
    def __init__(self, config, dataset, batch_size=1, dl_format=InputType.POINTWISE, shuffle=False, phase='train'):
        ...
        self.phase = phase
        if config['SSL_AUG'] in ['DuoRec','AdaptiveRec']  and self.phase == 'train':
    ...
    def data_preprocess(self):
        ...
        if self.config['model'] in ['DuoRec','AdaptiveRec']:
    ...
    def _shuffle(self):
        if self.real_time:
            ...
            if self.config['SSL_AUG'] in ['DuoRec','AdaptiveRec']and self.phase == 'train':
    ...
    def _get_processed_data(self, index):
        ...
        elif self.config['SSL_AUG'] in ['DuoRec','AdaptiveRec'] and self.phase == 'train':
